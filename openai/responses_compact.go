...
func compactionMessage(item ResponsesInputItem) (api.Message, string, error) {
	...
	case ResponsesToolSearchCall:
		if value.Execution == "server" {
			return api.Message{Role: "assistant", Content: "Tool search completed."}, "tool_search_call", nil
		}
		callID, err := clientToolSearchCallID(value.Execution, value.CallID, value.Type)
		if err != nil {
			return api.Message{}, "", err
		}
		arguments, err := toolCallArguments(value.Arguments)
		if err != nil {
			return api.Message{}, "", err
		}
		return api.Message{Role: "assistant", ToolCalls: []api.ToolCall{{
			ID: callID, Function: api.ToolCallFunction{Name: "tool_search", Arguments: arguments},
		}}}, "function_call", nil
	case ResponsesToolSearchOutput:
		...
		if value.Execution == "server" {
			return api.Message{Role: "assistant", Content: string(tools)}, "tool_search_output", nil
		}
		callID, err := clientToolSearchCallID(value.Execution, value.CallID, value.Type)
		if err != nil {
			return api.Message{}, "", err
		...
		return api.Message{
			Role: "tool", Content: string(tools), ToolName: "tool_search", ToolCallID: callID,
