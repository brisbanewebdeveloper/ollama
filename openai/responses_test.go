...
func TestUnmarshalResponsesInputItem(t *testing.T) {
	...
	t.Run("tool_search_call item", func(t *testing.T) {
		...
		if call.CallID == nil || *call.CallID != "call_search" || call.Execution != "client" {
			t.Fatalf("call = %#v", call)
		}
		if query, ok := call.Arguments["query"]; !ok || query != "order lookup" {
	...
	t.Run("tool_search_output item", func(t *testing.T) {
		...
		if output.CallID == nil || *output.CallID != "call_search" || len(output.Tools) != 1 {
...
func TestFromResponsesRequest_ToolSearchOutputBecomesToolContent(t *testing.T) {
	...
	if len(chat.Tools) != 2 || chat.Tools[0].Function.Name != "tool_search" || chat.Tools[1].Function.Name != "lookup_order" {
		t.Fatalf("native tools = %#v; discovered tool must be callable", chat.Tools)
	}
}
...
func TestFromResponsesRequest_ToolSearchOutputFlattensNamespaceMembers(t *testing.T) {
	...
	if len(chat.Tools) != 4 || findChatTool(chat.Tools, "mcp__openaiDeveloperDocs.search_openai_docs") == nil || findChatTool(chat.Tools, "mcp__codex_apps__github_search") == nil || findChatTool(chat.Tools, "plain") == nil {
		t.Fatalf("native tools = %#v; discovered tools must be callable", chat.Tools)
	}
}
