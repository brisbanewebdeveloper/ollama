...
func (w *WebSearchResponsesWriter) writeWebSearchResponse(final api.ChatResponse, calls []openai.ResponsesWebSearchCall, usage api.Metrics) error {
	...
	completedAt := time.Now().Unix()
	response.CompletedAt = &completedAt
	response.Output = buildResponsesWebSearchOutput(response.Output, w.preSearchThinking, w.preSearchContent, calls, w.otherToolCalls, w.req)
...
func buildResponsesWebSearchOutput(output []openai.ResponsesOutputItem, preSearchThinking, preSearchContent string, searchCalls []openai.ResponsesWebSearchCall, otherToolCalls []api.ToolCall, request openai.ResponsesRequest) []openai.ResponsesOutputItem {
	...
	items = append(items, openai.ResponsesToolCallOutputItems("mixed", otherToolCalls, request)...)
