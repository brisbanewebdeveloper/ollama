...
- Set a model default in a Modelfile: `PARAMETER think false`
...
<Note>Thinking is enabled by default in the CLI and API for supported models unless the model's Modelfile sets `PARAMETER think false`.</Note>
